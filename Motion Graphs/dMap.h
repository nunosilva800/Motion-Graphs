...
class dMap{
	...
	public:
	    ...
	    static void calculateTransformation(PointCloud *s1, PointCloud *s2, float *teta, float *x0, float *z0);
	...
	private:
		...
		void duplicateSpace();
		static void calculateSums(PointCloud *s1, PointCloud *s2, float *x1_, float *x2_, float *z1_, float *z2_, float *weights);
		static float calculateTeta1(PointCloud *s1, PointCloud *s2);
		static float calculateTeta2(float x1_, float x2_, float z1_, float z2_, float weights);
		static float calculateTeta3(PointCloud *s1, PointCloud *s2);
		static float calculateTeta4(float x1_, float x2_, float z1_, float z2_, float weights);
